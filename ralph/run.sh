...
if [ -z "$1" ]; then
  echo "Usage: ./ralph/run.sh <prd-issue-number>"
  exit 1
fi
...
echo "=== Ralph: starting work on PRD #${PRD_ISSUE} ==="
...
while true; do
  ...
  echo "=== ${open_issues} open sub-issue(s) remaining. Running once.sh... ==="
  echo "=== running once.sh ==="
  ./ralph/once.sh "${PRD_ISSUE}"
done
