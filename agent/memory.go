...
func RunMemoryAgent(ctx context.Context, p provider.Provider, home homer.Homer, messages []provider.Message, currentMemory string) error {
	writeMemoryTool := tool.NewTool("write_memory",
		...
		func(ctx context.Context, in WriteMemoryInput) (WriteMemoryOutput, error) {
			...
			if err := home.Upsert("MEMORY.md", []byte(in.Content)); err != nil {
